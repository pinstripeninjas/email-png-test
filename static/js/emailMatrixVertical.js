...
const criteriaHeader = document.querySelector(".criteria-header");
const criteriaContent = document.querySelector(".criteria-content");
	...
	axios
		...
		.then(
			axios.spread(({ data: forecast }, { data: criteria }) => {
				// create matrix header and content
				buildHeader(forecast);
				buildMatrixContent(forecast, criteria);
				// create criteria header and content
				buildCriteriaHeader(criteria);
				buildCriteriaContent(forecast, criteria);
			})
		)
		.then(() => {
...
function buildCriteriaHeader(criteria) {
	const title = document.createElement("div");
	title.classList.add("field-name");
	criteriaHeader.append(title);
	...
	criteria.header.map((field) => {
		...
		return criteriaHeader.append(firstDiv);
	});
}

function buildCriteriaContent(forecast, criteria) {
	forecast.forecast.map((field, i) => {
		const finalDiv = document.createElement("div");
		finalDiv.classList.add("matrix-row");
		const fieldNameDiv = document.createElement("div");
		...
		return criteriaContent.append(finalDiv);
	});
}
